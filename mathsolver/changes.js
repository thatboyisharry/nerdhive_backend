...
// e.g. x^2+5x+6 -> x^2 + 2x + 3x + 6
Change.changeFormatFunctionMap[ChangeTypes.BREAK_UP_TERM] = function(step) {
  return `\\text{${Change.ChangeText[step.changeType]}}`;
};
...
// e.g. (x+3)(x+3)
Change.changeFormatFunctionMap[ChangeTypes.FACTOR_SUM_PRODUCT_RULE] = function(step) {
  return `\\text{${Change.ChangeText[step.changeType]}}`;
};
// e.g. 2 = x -> x = 2
...
// e.g. 2 = x -> x = 2
Change.changeFormatFunctionMap[ChangeTypes.FACTOR_SYMBOL] = function(step) {
  return `\\text{${Change.ChangeText[step.changeType]}}`;
};
...
Change.ChangeText = {
  FIND_ROOTS:'Find the roots of the equation',
  FACTOR_SYMBOL:'Factorise',
  ...
  BREAK_UP_TERM:'Break up the middle term',
  ...
  FACTOR_SUM_PRODUCT_RULE:'Apply the factor sum product rule',
